...
虚拟摄像头功能位于 `实用 -> 美化`，默认关闭，只安装在微信主进程。设置页通过 Android 系统文件选择器选择图片或视频，并把文件复制到微信数据目录下的 `files/Hchat/virtual_camera/`，避免目标进程无法继续读取临时 `content://` URI；开启、关闭或更换媒体后需强制停止并重启微信。Camera1 路径会把硬件预览重定向到受控 `SurfaceTexture`，把所选媒体输出到微信原预览 Surface，同时替换 NV21 预览回调和 Camera1 拍照 JPEG。Camera2 路径会同时改写 `CameraDeviceImpl` 的 capture-session 输出列表、`OutputConfiguration` / `SessionConfiguration` 以及 `CaptureRequest.Builder.addTarget/removeTarget`，每个原始预览或录像 Surface 都映射到独立的虚拟输出并持续消费真实相机帧；已识别的 YUV `ImageReader` 使用 `ImageWriter` 写入虚拟帧，JPEG/DEPTH_JPEG 拍照输出保留原始链路。运行时在相机停止、会话重建和功能销毁时释放 `MediaPlayer`、EGL、`SurfaceTexture`、`Surface` 与 `ImageWriter`。当前 Activity 类名包含 `face`、`facerecognize`、`huiyan`、`identity`、`certification`、`verify`、`liveness`、`realname`、`soter` 或 `biometric` 等身份/人脸/活体标识时强制旁路，避免改变认证摄像头输入。微信 8.0.77(3160) APK 已静态确认使用 `CameraManager`、`CameraCaptureSession`、`CaptureRequest.Builder`、`ImageReader` 及 `com.tencent.mm.media.camera.instance.camera2.Camera2Impl` / CameraKit 视图链；尚未完成 8.0.77 真机验证，也没有 8.0.49、8.0.58、8.0.66、8.0.68、8.0.72、8.0.74、8.0.76 的对应 APK 验证。本实现参考 GPL-3.0 项目 `zensu357/Android-CamSwap-OpenSource` 的 Camera1/Camera2 Surface 重定向架构，在 Hchat 内重新实现，并沿用仓库 GPL-3.0 许可。
...
- 微信原生分享页的“最近聊天”和“最近转发”只过滤 `wxid_hchat_group_*` 虚拟分组入口；真实会话未被持久归拢，本来就在原生分享结果中，不再执行取消根 `parentRef` 限制的第二次展开查询，也不复制完整结果到 `MatrixCursor`。搜索、完整联系人页、选择状态、确认弹窗和发送链路保持微信原生行为。
...
撤回提示优先紧跟原消息插入；原消息时间缺失或指定时间 Hook 不可用时，使用已有本地系统消息 API 按微信默认时间插入，不能直接丢弃提示。缺失的原发送时间变量保持为空，模板展开后全空时使用默认提示。原消息只按会话与服务端消息 ID 匹配，校验会话、ID 和非撤回状态，不把服务端 ID 当作本地 msgId 查询。提示接口返回失败或抛异常时仍拦截撤回，但不标记该事件已完成，后续同一事件再次触发可以重试；这不代表存在定时自动重试。降级与首次接口调用成功各记录一次日志，成功日志仅说明调用已提交，实际展示仍需设备验证。本次修正基于模块源码，未新增微信内部定位；8.0.77 及其它版本的真机提示显示尚未验证。
