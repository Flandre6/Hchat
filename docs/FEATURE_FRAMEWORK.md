...
功能还会从已确认的消息入库参数类派生单字符串 setter 候选，仅在参数确实是完整 `msgsource` 且消息类型匹配时
合并标记，使媒体发送链路在提交请求前也能获得标记。`反安全消息` 会关闭微信对该标记的识别检查，并在已确认的本地消息入库
...
安全消息限制。微信原生转发链路不清理
原消息的安全节点，继续保留微信原有的安全消息行为：界面可能显示转发完成，但接收方不可见该消息。
