...
`special_message` 使用精确的微信版本名和完整 versionCode 选择宿主配置：8.0.76/3140
继续使用 `lq1.u0.r(a65.en4,e9)`、`lq1.d1.a(e9)`、`o22.y` 与 `N0()`；8.0.77/3160
使用 `ft1.u0.p(y85.gq4,e9)`、`ft1.d1.a(e9)`、`k52.y` 与 `Q0()`。8.0.77 的
`com.tencent.mm.storage.e9.G` 是 `msgSource` 写入字段，表情请求分发前会再次补入标记。
未知版本不安装 Hook，不把本次静态适配宣称为全版本兼容。版本选择回归位于
`scripts/run_safety_message_tests.cjs`，静态证据和验证边界见 `docs/SAFETY_MESSAGE_8077.md`；
目前仍需在 8.0.77 真机验证名单内文字、链接、表情及名单外消息。
