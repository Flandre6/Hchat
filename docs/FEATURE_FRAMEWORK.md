...
- `滑动手势` 属于实用功能，设置页包含 `左滑引用回复`、`右滑复读` 和 `长按菜单复读` 三个独立开关，默认都关闭；用户可以只启用其中一种复读触发方式。消息行定位优先 hook `MicroMsg.ChattingDataAdapterV3` 的 `_onBindViewHolder` 绑定方法，保存当前行根 View、talker、本地 msgId 和适配器里的原生消息对象；8.0.74 这类版本的适配器 item 是包装对象，必须递归拆出内部 `com.tencent.mm.storage.*` 原生消息对象。
...
- `沉浸式背景` 属于实用功能，设置入口暂不在设置页展示，默认关闭。聊天页、微信首页会话页、通讯录、发现和我五个位置可分别选择一张本地图片并独立设置 `10%..100%` 背景透明度；图片通过系统 `image/*` 文件选择器校验后原子保存到 Hchat 私有目录 `Hchat/background_beauty`，运行时异步按屏幕用途采样解码并缓存。五个位置共用生命周期和图片加载逻辑，但各自使用独立图片与透明度。首页四页不再向 `CustomViewPager` 插入额外子 View，而是在微信原生 Fragment 根 View、列表表面和共享顶栏宿主上直接设置按屏幕坐标对齐的 Drawable；只清理当前页面浅层的 `RecyclerView/ListView/ScrollView`、ActionBar/Toolbar、`AppBrandDesktopContainerView`、`ChatFooterCustom` 以及逆向确认资源 `cj1`、`gxp`、`gxn`、`hxi` 等白名单表面，不拦截 Android 全局背景写入，不跳过微信动态背景绘制，不隐藏整个动态背景容器，也不进入 Compose 子树或遍历清空全部页面节点。微信、通讯录、发现和我分别通过 `MainUI`、`MvvmAddressUIFragment`、`FindMoreFriendsUI`、`MoreTabUI` 的原生布局创建入口记录实际根 View，恢复时只通过缓存根 View 或 `Fragment#getView()` 取当前节点，不主动再次调用 `getLayoutView()`；根 View 必须同时匹配当前 Activity 和 slot，避免切页时序差把新背景写到旧页。聊天页通过 DexKit 双字符串锚点 `MicroMsg.ChattingUI.ChattingBackgroundComponent` / `initBackground:` 定位并缓存原生背景初始化方法，再通过组件字段 `d` 的 `c(int): View` 按稳定资源 `bka` 获取主背景 ImageView，字段 `f` 仅作为同一原生主背景的已确认兜底，不再反射选择“第一个 ImageView”。运行时不在内容层上方放置触摸覆盖层，不向 ComposeView 添加子 View；透明 Drawable 写入前后保留原 padding 与最小尺寸，关闭功能、移除图片、切换到未配置图片的页面或 Activity 销毁时恢复原生背景、前景、tint、图片状态和系统栏状态。背景裁剪区域只在挂载或尺寸变化时计算，重复生命周期刷新复用已应用状态，不在列表每帧绘制时查询屏幕坐标。`8.0.49`、`8.0.58`、`8.0.66`、`8.0.68`、`8.0.72`、`8.0.74`、`8.0.76` 已横向确认四页 Fragment 类与布局生命周期入口稳定，聊天双字符串每版均唯一命中无参 `void` 原生背景初始化方法，且上述 `d.c(bka)`、主背景字段 `f` 与辅助可见性字段 `g` 的语义在七版一致。
