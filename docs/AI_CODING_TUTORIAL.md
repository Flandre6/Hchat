...
微信 APK 放在哪里、文件叫什么都由开发者自己决定。传给 DexClub 前先设置绝对路径变量：
...
不要把 `./weixin.apk`、只含文件名的相对路径或个人设备上的固定归档目录写进项目文档和代码。需要切换 APK 时只修改本机的环境变量。
...
把 APK 放到 Termux 可读取的位置，自行设置绝对路径并确认文件存在：
